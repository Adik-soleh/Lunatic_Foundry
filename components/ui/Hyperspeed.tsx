...
import { Distortion, HyperspeedOptions } from '@/types';
...
// ─── Default fallback distortion (simple) ─────────────────────────────────────

const fallback_distortion_vertex = `
  #define PI 3.14159265358979
...
`;
const fallback_distortion_uniforms = {
...
const roadMarkings_fragment = `
  ...
  float brokenLines=step(1.0-brokenLineWidth,fract(uv.x*2.))*step(laneEmptySpace,fract(uv.y*10.));
  float sideLines=step(1.0-brokenLineWidth,fract((uv.x-laneWidth*(uLanes-1.))*2.))+step(brokenLineWidth,uv.x);
...
class CarLights {
  ...
  colors: number[] | THREE.Color; speed: [number, number];
  fade: THREE.Vector2; mesh!: THREE.Mesh;
  constructor(w: App, o: HyperspeedOptions, c: number[] | THREE.Color, s: [number, number], f: THREE.Vector2) {
    this.webgl = w; this.options = o; this.colors = c; this.speed = s; this.fade = f;
  }
  init() {
    ...
    const colorArray = Array.isArray(this.colors) ? (this.colors as number[]).map(c => new THREE.Color(c)) : [new THREE.Color(this.colors as unknown as number)];
    ...
    const dist = this.options.distortion as Distortion;
    const material = new THREE.ShaderMaterial({
      fragmentShader: carLightsFragment,
      vertexShader: carLightsVertex,
      transparent: true,
      uniforms: Object.assign({ uTime: { value: 0 }, uTravelLength: { value: o.length }, uFade: { value: this.fade } }, this.webgl.fogUniforms, dist?.uniforms || {}),
    });
    material.onBeforeCompile = s => { s.vertexShader = s.vertexShader.replace('#include <getDistortion_vertex>', dist?.getDistortion || ''); };
...
class LightsSticks {
  ...
  init() {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      uniforms: Object.assign({ uTravelLength: { value: o.length }, uTime: { value: 0 } }, this.webgl.fogUniforms, dist?.uniforms || {}),
    });
    material.onBeforeCompile = s => { s.vertexShader = s.vertexShader.replace('#include <getDistortion_vertex>', dist?.getDistortion || ''); };
    const mesh = new THREE.Mesh(instanced, material);
    mesh.frustumCulled = false;
...
class Road {
  ...
  createPlane(side: number, _width: number, isRoad: boolean) {
    ...
    const material = new THREE.ShaderMaterial({
      ...
      side: THREE.DoubleSide,
      uniforms: Object.assign(uniforms, this.webgl.fogUniforms, dist?.uniforms || {}),
    });
    material.onBeforeCompile = s => { s.vertexShader = s.vertexShader.replace('#include <getDistortion_vertex>', dist?.getDistortion || ''); };
...
class App {
  ...
  constructor(container: HTMLElement, options: HyperspeedOptions) {
    ...
    // Make canvas fill its container absolutely
    ...
    // Aim camera down the road (road extends in -z direction)
  ...
  update(delta: number) {
    ...
    const fovChg = lerp(this.camera.fov, this.fovTarget, lp);
    if (fovChg !== 0) { this.camera.fov += fovChg * delta * 6; this.camera.updateProjectionMatrix(); }
    const dist = this.options.distortion as Distortion;
    if (dist?.getJS) {
...
const Hyperspeed: FC<HyperspeedProps> = ({ effectOptions = {} }) => {
  ...
  useEffect(() => {
    ...
    // Resolve string distortion names
    if (typeof options.distortion === 'string') {
      options.distortion = distortions[options.distortion] ?? {
        uniforms: fallback_distortion_uniforms,
        getDistortion: fallback_distortion_vertex,
      };
    } else if (!options.distortion) {
      options.distortion = { uniforms: fallback_distortion_uniforms, getDistortion: fallback_distortion_vertex };
    }
